...
const char* ts3plugin_name() {
	...
	if(!result) {
		const wchar_t* name = AJE_PLUGIN_NAME_L;
		if(wcharToUtf8(name, &result) == -1) {  /* Convert name into UTF-8 encoded result */
			return AJE_PLUGIN_NAME;  /* Conversion failed, fallback here */
		}
	}
	return result;
#else
	return AJE_PLUGIN_NAME;
#endif
}

/* Plugin version */
const char* ts3plugin_version() {
    return AJE_PLUGIN_VERSION;
}
...
const char* ts3plugin_author() {
    ...
    return AJE_PLUGIN_AUTHOR;
}

/* Plugin description */
const char* ts3plugin_description() {
	/* If you want to use wchar_t, see ts3plugin_name() on how to use */
    return AJE_PLUGIN_DESCRIPTION;
}
...
void ts3plugin_initMenus(struct PluginMenuItem*** menuItems, char** menuIcon) {
	...
	CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_CHANNEL, MENU_ID_CHANNEL_SAVE_RAID, "Сохранить рейд", "");
...
// Кодирует строку что бы можно было использовать в запросе.
// Все что не буквы заменяет на %xx (код символа в hex)
